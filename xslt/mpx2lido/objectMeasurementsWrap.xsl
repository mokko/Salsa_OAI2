...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:template match="mpx:maßangabe">
  <lido:objectMeasurementsSet>
   <lido:displayObjectMeasurements xml:lang="de"
    lido:encodinganalog="mpx:maßangabe">
    <xsl:if test="@typ">
 ...
 <xsl:template name="Objektmaß">
  ...
  <!--
   sometimes people write stupid strings in this field, so test if value1 is a number
   see http://stackoverflow.com/questions/3854345/xpath-test-if-is-number
  -->
  <xsl:if test="number ($value1) = number($value1)">
