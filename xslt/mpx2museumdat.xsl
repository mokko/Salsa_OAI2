<xsl:stylesheet version="1.0"
	...
	<xsl:template match="mpx:sammlungsobjekt">
		...
		<xsl:message ><xsl:value-of select="$currentId"/></xsl:message>
		<museumdat:museumdat>
			<museumdat:descriptiveMetadata>
				...
				<museumdat:identificationWrap>
					...
					<museumdat:repositoryWrap>
						<museumdat:repositorySet
							...
							<xsl:if
								test="child::mpx:verantwortlichkeit or mpx:ständigerStandort">
				...
				<museumdat:descriptionWrap>
					<xsl:choose>
						<xsl:when
							test="child::mpx:personKörperschaftRef[@funktion='Hersteller']">
						...
						<xsl:when
							test="child::mpx:geogrBezug[@bezeichnung='Ethnie|Kultur']">
							<museumdat:displayCreator
								museumdat:encodinganalog="mpx:personKörperschaftRef">
								<xsl:for-each
					...
					<museumdat:displayCreationDate
						...
						<xsl:for-each
							...
							<xsl:if test="@art">
								<xsl:value-of
									select="concat(' (',normalize-space(@art),')')" />
							</xsl:if>
						...
						<xsl:when
							...
							<museumdat:displayMeasurements
								...
								<xsl:for-each
									...
									<xsl:if test="@typ">
										<xsl:value-of
											select="concat(' (',replace(@typ,'Objektmaß','Höhe x Breite x Tiefe'),')')" />
									</xsl:if>
								</xsl:for-each>
							</museumdat:displayMeasurements>
						</xsl:when>
					...
					<museumdat:descriptiveNoteWrap>
						<xsl:if
							test="child::mpx:kurzeBeschreibung">
						...
						<xsl:if
							test="child::mpx:ikonographischeBeschreibung">
							<museumdat:descriptiveNoteSet
						...
						<!-- exists only for records with objekttyp="audio" -->
						<xsl:if test="child::mpx:besetzung">
							<museumdat:descriptiveNoteSet
								museumdat:type="content">
								<xsl:for-each
				...
				<museumdat:eventWrap>
					<museumdat:indexingEventWrap>
						...
						-->
						<xsl:if
							test="not (mpx:verantwortlichkeit = 'EM-Medienarchiv')">
							<xsl:if
						...
						<xsl:if
							test="mpx:verantwortlichkeit = 'EM-Am Archäologie'">
							<xsl:if
								test="child::mpx:datierung/@vonJahr or child::mpx:datierung/@vonJahr">
								<museumdat:indexingEventSet>
									<!-- 1. EVENTTYPE -->
									<museumdat:eventType>
										Herkunft
									...
									<museumdat:indexingDates>
										<xsl:if
											test="child::mpx:datierung/@vonJahr">
											...
											<museumdat:earliestDate
												...
												<xsl:if
													test="child::mpx:datierung/@vonMonat">
													-
													<xsl:value-of
														select="child::mpx:datierung/@vonMonat" />
													<xsl:if
														test="child::mpx:datierung/@vonTag">
														-
														<xsl:value-of
						...
						<xsl:if
							test="mpx:verantwortlichkeit = 'EM-Medienarchiv'">
							<xsl:if
					...
					<xsl:if
						test="child::mpx:geogrBezug[@bezeichnung='Ethnie|Kultur|Sprachgruppe']">
					...
					<xsl:for-each
						...
						<xsl:if
							...
							<museumdat:resourceSet>
								...
								<xsl:if
									test="mpx:multimediaInhaltAnsicht">
								...
								<xsl:if
									test="mpx:multimediaUrhebFotograf">
